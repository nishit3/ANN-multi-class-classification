...
n_experiments = 50
accuracies_with_batchnorm = np.zeros(n_experiments)
accuracies_without_batchnorm = np.zeros(n_experiments)
...
for experiment in range(n_experiments):
    ...
    accuracies_with_batchnorm[experiment] = accuracy
...
for experiment in range(n_experiments):
    ...
    result = result[result == labels]
    accuracy = len(result) / len(labels) * 100
    accuracies_without_batchnorm[experiment] = accuracy

plt.plot(np.linspace(1, n_experiments, n_experiments), accuracies_with_batchnorm)
plt.ylabel('Accuracy %')
plt.xlabel('Experiment No.')
plt.title("With Batch Normalization")
plt.show()

plt.plot(np.linspace(1, n_experiments, n_experiments), accuracies_without_batchnorm)
plt.ylabel('Accuracy %')
plt.xlabel('Experiment No.')
